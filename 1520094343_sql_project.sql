...
WHERE CONCAT (LEFT(b.starttime,4),'-',SUBSTR(b.starttime,6,2),'-',SUBSTR(b.starttime,9,2)) = '2012-09-14'
...
SELECT sub2.facility_name,
	   sub2.tot_revenue
FROM (
SELECT sub.facility_name,
		SUM(sub.revenue) AS tot_revenue
FROM (
    SELECT f.name AS facility_name,
